...
def lambda_handler(event, context):
    ...
    prompt = f"Analyse the following keyword(s) {', '.join(keywords)} to see if they are related to the domain / webpage {domain}. Only analyse kewyords that are provided. If only one keyword phrase is provided, only analyse one. Be very strict about this, it's better to have false negatives than false positives.  For each keyword, respond with a JSON object containing the 'keyword' and 'related' fields, where 'related' is a boolean (true or false). Return a single JSON array containing these objects."
