...
function ProjectsSection() {
  ...
  return (
    <section>
      ...
      {atLimit && (
        <p className="mt-2 text-sm text-muted">You've hit your project limit. <Link to="/pricing" className="text-brand-600">Upgrade</Link> for more.</p>
      )}
      ...
      <div className="mt-6 space-y-2">
        ...
        {projects.map((p) => (
          <div key={p.projectId} className={`card flex items-center gap-3 p-4 ${p.projectId === activeId ? 'ring-2 ring-brand-400' : ''}`}>
            ...
            <button onClick={() => remove(p.projectId)} className="text-sm text-faint hover:text-red-600">Delete</button>
          </div>
        ))}
      </div>
    </section>
