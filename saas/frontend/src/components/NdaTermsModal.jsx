    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface shadow-2xl">
        <div className="flex items-center justify-between gap-3 border-b border-line px-6 py-4">
          <div>
            <h2 className="text-base font-bold text-heading">{AGREEMENT_TITLE}</h2>
            {showVersion && <p className="mt-0.5 text-xs text-faint">Version {NDA_VERSION} · exactly what trial users see and what the Acceptance Record PDF embeds</p>}
          </div>
          <button onClick={onClose} aria-label="Close" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-sunken text-muted hover:bg-overlay"><X size={18} /></button>
        </div>
        <div className="space-y-3 overflow-y-auto px-6 py-5 text-sm leading-relaxed text-body">
          {AGREEMENT_INTRO.map((intro, i) => (
            <p key={i} className={intro.boxed ? 'rounded-lg border border-line bg-raised p-3' : undefined}>
              {intro.text}
            </p>
          ))}
