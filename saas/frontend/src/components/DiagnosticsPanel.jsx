...
export default function DiagnosticsPanel({ diagnostics: d }) {
  ...
  const env = d.env || {};
  return (
    <div className="mt-3 rounded-xl border border-line bg-raised">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm font-medium text-body">
        🔧 Diagnostics
        <span className="ml-auto text-xs text-faint">{open ? 'Hide' : 'Show'}</span>
      </button>
      {open && (
        <div className="space-y-3 border-t border-hair px-3 py-3 text-xs text-dim">
          {env.url && (
            <div>
              <div className="font-semibold text-body">Environment</div>
          ...
          {d.apiFailures?.length > 0 && (
            <div>
              <div className="font-semibold text-body">Failed actions</div>
          ...
          {d.apiSuccesses?.length > 0 && (
            <div>
              <div className="font-semibold text-body">Other calls that succeeded around the same time</div>
              <ul className="mt-1 space-y-0.5 font-mono text-[11px] text-muted">
          ...
          {d.errors?.length > 0 && (
            <div>
              <div className="font-semibold text-body">Errors</div>
          ...
          {d.errorToasts?.length > 0 && (
            <div>
              <div className="font-semibold text-body">Messages the user saw</div>
              <ul className="mt-1 space-y-0.5 text-muted">{d.errorToasts.map((t, i) => <li key={i}>{t.message}</li>)}</ul>
            </div>
          )}
          {d.fields?.length > 0 && (
            <div>
              <div className="font-semibold text-body">Form fields filled in</div>
              <ul className="mt-1 space-y-0.5 text-muted">
                {d.fields.map((f, i) => <li key={i} className="break-words"><span className="font-medium text-dim">{f.label}:</span> {f.value}</li>)}
              </ul>
            </div>
          )}
        </div>
