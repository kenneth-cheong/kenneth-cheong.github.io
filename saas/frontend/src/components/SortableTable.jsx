    ...
    <div className="overflow-auto" style={{ maxHeight }}>
      <table className={`w-full text-left text-sm ${className}`}>
          ...
          <tr>
            {cols.map((c) => (
              <th
                key={c.key}
                onClick={() => onSort(c)}
                className={`sticky top-0 z-10 bg-slate-50 px-3 py-2 font-medium text-slate-500 ${c.align === 'right' ? 'text-right' : ''} ${c.sortable === false ? '' : 'cursor-pointer select-none hover:text-slate-700'}`}
              >
                {c.label}
                {sort.key === c.key && <span aria-hidden> {sort.dir > 0 ? '▲' : '▼'}</span>}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {sorted.map((row, i) => (
            <tr key={rowKey ? rowKey(row, i) : i} className={`border-t border-slate-100 ${zebra && i % 2 ? 'bg-slate-50/50' : ''}`}>
              {cols.map((c) => (
